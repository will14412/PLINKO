...
class SimpleButton extends Widget {
  ...
  public SimpleButton(String uiname, int x, int y, String uilable) {
    ...
    this.bounds = new SimpleUIRect(x, y, x + this.widgetWidth, y + this.widgetHeight);
  ...
  public boolean handleMouseEvent(String mouseEventType, int x, int y) {
    if ( isInMe(x + 45, y + 24) /*&& (mouseEventType.equals("mouseMoved") || mouseEventType.equals("mousePressed"))*/) {
      rollover = true;
    } else { 
      rollover = false;
    }
  ...
  public void drawMe() {
    ...
    textSize(textSize);
    strokeWeight(1);
    text(this.UILabel, locX+textPad - 5, locY+textPad - 7, widgetWidth, widgetHeight);
    popStyle();
...
class Slider extends Widget {
  ...
  public void drawMe() {
    ...
    rect(bounds.left + 52, bounds.top + 15, bounds.getWidth(), bounds.getHeight());
    fill(SimpleUITextColor);
    textSize(17);
    text(this.UILabel, bounds.left+textPad + 48, bounds.top - 16);
  ...
  void drawValue() {
    ...
    text(currentValueString, bounds.right-60, bounds.top + 16);
    popStyle();
  }
